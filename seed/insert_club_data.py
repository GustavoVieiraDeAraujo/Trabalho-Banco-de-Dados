...
def insert_club_data(clubs_data):
  if not clubs_data:
    print("Nenhum clube para inserir.")
    return
  query = """INSERT INTO Time (id, imagem_escudo, nome, apelido, data_fundacao, site_time_url,quantidade_socios, quantidade_jogadores_selecao, valor_mercado,id_estadio, id_localizacao) VALUES %s ON CONFLICT (id) DO NOTHING;"""
  conn = connect_postgresql_database()
  cursor = conn.cursor()
  try:
    execute_values(cursor, query, clubs_data)
    conn.commit()
    print(f"{len(clubs_data)} clubes inseridos com sucesso!")
...
#   if address_line2:
#     if "," in address_line2:
#       cidade = address_line2.split(",")[0].strip()
#     else:
#       cidade = address_line2.partition(" ")[2].strip()
#   return (
#     int(club_data["id"]),
#     imagem_bytes,
